...
async function main() {
  ...
  if (utils.isAddress(paymasterAddress)) {
    ...
    */
    const op = await primeSdk.estimate({ url: `${arka_url}?apiKey=${arka_api_key}&chainId=${Number(process.env.CHAIN_ID)}`, context: { mode: 'sponsor', validAfter: new Date().valueOf(), validUntil: new Date().valueOf() + 6000000 } });
